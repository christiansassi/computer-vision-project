...
def FindMatches(BaseImage, SecImage, Value):
...
def StitchImages(BaseImage, SecImage, Value):
    # Finding matches between the 2 images and their keypoints
    Matches, BaseImage_kp, SecImage_kp = FindMatches(BaseImage, SecImage, Value)
...
if __name__ == "__main__":
    ...
    Image1 = cv2.imread(f"images/{section}/center_left_f500.jpg")
    Image2 = cv2.imread(f"images/{section}/center_right_f500.jpg")
    ...
    StitchedImage = StitchImages(Image1, Image2, 0.4)

    # Displaying the stitched images.
    # cv2.imshow("stitched", resize(StitchedImage, StitchedImage.shape[1], StitchedImage.shape[0]))
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    
    cv2.imwrite(f"stitched/stitched_{section}.png", StitchedImage)    