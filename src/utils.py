...
def crop_image(mat: cv2.typing.MatLike | cv2.cuda.GpuMat | cv2.UMat) -> np.ndarray:

    _mat = mat.copy()

    height, width = _mat.shape[:2]

    center_x = width // 2
    center_y = height // 2

    _mat = _mat[center_y - 878:center_y + 879, center_x - 878:center_x + 879]
...
def extract_frame(video: str | cv2.VideoCapture, frame_number: int) -> np.ndarray:
    ...
    # Open video if video is a path to a video file
    ...
    # If video is a video object, save its current frame position so we can restore it later
...
def black_box_on_image(left_frame: cv2.typing.MatLike | cv2.cuda.GpuMat | cv2.UMat, right_frame: cv2.typing.MatLike | cv2.cuda.GpuMat | cv2.UMat, left_width: int = None, left_height: int = None, right_width: int = None, right_height: int = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    if left_width is None or left_width > _left_frame.shape[1]:
        left_width = _left_frame.shape[1] - 1

    if left_height is None or left_height > _left_frame.shape[0]:
        left_height = _left_frame.shape[0] - 1
    
    if right_width is None or right_width > _right_frame.shape[1]:
        right_width = _right_frame.shape[1] - 1

    if right_height is None or right_height > _right_frame.shape[0]:
        right_height = _right_frame.shape[0] - 1

    # Left frame
    x, y = 0, 0
    _left_frame[y:y + left_height + 1, x:x + left_width + 1] = 0
    
    # Right frame
    x, y = right_width, 0
    _right_frame[y:y + right_height + 1, x:] = 0
